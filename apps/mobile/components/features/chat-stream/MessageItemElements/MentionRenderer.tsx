...
// Top-level custom mention renderer
export const CustomMentionRenderer: CustomTextualRenderer = ({
    ...
    if (attributes?.['data-type'] === 'commandMention') {
        return <CommandMentionRenderer commandName={attributes?.['data-id']} TDefaultRenderer={TDefaultRenderer} {...props} />
    }
...
const CommandMentionRenderer = ({
    commandName,
    TDefaultRenderer,
    ...props
}: CustomRendererProps<TText | TPhrasing> & { commandName: string }) => {
    ...
    // @ts-ignore
    return <TDefaultRenderer {...props} onPress={handleCommandPress} />;
};
